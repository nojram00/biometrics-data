...
class NewRecords{
     ...
     * @param string $ip
    **/
    public static function NewFindRecords($pass, $id = null, $startTime = null, $endtime= null, $ip){
    ...
    public function GetTimeOutWithinThisDate($date){
        ...
        foreach(self::$response as $r){
            $dateFormat = date('Y-m-d', $r['time']/1000);
            if($dateFormat == $date && $r['attendance']['attendanceStatus'] == 'Time Out'){
                return date('H:i', $r['time']/1000);
            }
        }
    }
